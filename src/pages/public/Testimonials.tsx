      ...
      <div className="max-w-[1440px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {REVIEWS.map((review, i) => (
            <div key={i} className="border-[3px] border-[#121212] bg-white p-8 md:p-10 shadow-[6px_6px_0px_0px_#121212] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all flex flex-col">
              ...
              <div className="border-t-[3px] border-[#121212] pt-6 mt-auto">
                 <h4 className="font-black text-sm uppercase tracking-widest text-[#121212]">{review.author}</h4>
                 <div className="flex justify-between items-center mt-2">
                   <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#9E1B1D]">{review.role}</p>
                   <div className="flex gap-1">
                     {[...Array(5)].map((_, j) => <Star key={j} size={12} fill="#F4BF4B" className="text-[#F4BF4B]"/>)}
                   </div>
                 </div>
              </div>
            </div>
