...
export const Destinations = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FCFBF7] pt-24 md:pt-32 pb-24 px-[clamp(1rem,4vw,3rem)] nfa-texture selection:bg-nfa-gold">
      ...
      {/* Header */}
      <div className="max-w-[1440px] mx-auto mb-16 border-b-[4px] border-[#121212] pb-8">
        <div className="flex items-center gap-2 text-[#9E1B1D] mb-4">
           <Crosshair size={14} /> 
           <span className="font-black uppercase text-[10px] tracking-[0.3em]">Sector Registry // Operations</span>
        </div>
      ...
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 xl:grid-cols-[250px,1fr] gap-12">
             ...
             <div className="flex flex-wrap xl:flex-col gap-2">
               {["ALL", "NORDIC", "ASIA", "SOUTH AMERICA"].map(region => (
                 <button
                   ...
                   className={`px-4 py-3 text-xs font-black uppercase tracking-widest border-[2px] transition-all w-full text-left ${
                     filterRegion === region 
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8">
          {filtered.map(dest => (
             <RegistryCard key={dest.id} dest={dest} />
          ))}
        </div>
