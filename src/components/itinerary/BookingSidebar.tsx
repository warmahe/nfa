...
import { Calendar, Users, ArrowRight, ShieldCheck, Info, Zap } from 'lucide-react';
...
export const BookingSidebar = ({ pkg, travelers, setTravelers, isEditing, onUpdate }: any) => {
  ...
  const maxSlots = pkg?.maxSlots || pkg?.availability?.maxSlots || 12;
  const bookedSlots = pkg?.bookedSlots || 0;
  ...
  const total = (priceInt * travelers) + 150 + addOnsTotal;
  ...
  return (
    <div className="sticky top-28 border-[4px] border-[#121212] bg-white p-6 md:p-8 shadow-[12px_12px_0px_0px_#121212]">
      ...
      {/* Availability Badge */}
      {!isEditing && (
        <div className="mb-6 pb-4 border-b-2 border-[#F4BF4B]">
          <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 border-2 ${availableSlots > 3 ? 'bg-green-50 border-green-400 text-green-700' : availableSlots > 0 ? 'bg-yellow-50 border-yellow-400 text-yellow-700' : 'bg-red-50 border-red-400 text-red-700'}`}>
      ...
      <div className="mb-10 pb-6 border-b-2 border-gray-100">
        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#121212]/40 block mb-2 text-center">Price Per Person</span>
        {isEditing ? (
           <div className="flex flex-col items-center gap-4">
              ...
              <span className="text-[8px] font-black uppercase text-[#9E1B1D]">Edit Price</span>
           </div>
        ) : (
      ...
      <div className="space-y-6 mb-10">
        ...
        <div className="flex flex-col gap-3">
           ...
           <div className="flex border-2 border-[#121212] h-16 bg-[#121212]">
              ...
              <button onClick={() => setTravelers(Math.min(availableSlots, travelers + 1))} className="w-20 text-white hover:bg-[#F4BF4B] hover:text-[#121212] font-black text-2xl transition-colors">+</button>
           </div>
           {travelers > availableSlots && (
             <p className="text-[9px] text-[#9E1B1D] font-black uppercase">⚠ Only {availableSlots} slots available</p>
           )}
        </div>
      </div>
      ...
      {(pkg?.predefinedAddons?.length > 0) && (
        <div className="mb-8 pb-6 border-b-2 border-gray-200">
          <h4 className="text-[10px] font-black uppercase tracking-widest mb-4 flex items-center gap-2 text-[#121212]">
            ...
            {pkg.predefinedAddons.map((addon: any) => (
              <label key={addon.id} className="flex items-start gap-3 cursor-pointer hover:bg-[#FCFBF7] p-2 rounded transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[9px] font-bold text-[#9E1B1D] mt-2">+{addon.price} {currency}</p>
                </div>
              </label>
            ))}
          </div>
      ...
      <div className="bg-[#FCFBF7] border-2 border-[#121212] p-6 mb-8 space-y-4">
         ...
         {addOnsTotal > 0 && (
           <div className="flex justify-between font-bold text-xs uppercase tracking-tight">
              <span className="opacity-50">Add-ons Total</span>
              <span className="text-[#9E1B1D]">{addOnsTotal.toLocaleString()} {currency}</span>
           </div>
         )}
      ...
      {!isEditing && (
        <Link 
          to={`/booking/${pkg.id}?travelers=${travelers}&addons=${selectedAddOns.join(',')}`} 
          className={`w-full py-6 font-black text-xs uppercase tracking-[0.4em] flex justify-center items-center gap-3 transition-all shadow-[6px_6px_0px_0px_#F4BF4B] active:translate-x-1 active:translate-y-1 active:shadow-none ${
            travelers > availableSlots 
              ? 'bg-gray-400 text-white cursor-not-allowed opacity-50' 
              : 'bg-[#121212] text-[#F4BF4B] hover:bg-[#9E1B1D] hover:text-white'
          }`}
          onClick={(e) => travelers > availableSlots && e.preventDefault()}
        >
