...
import { Mail, Phone, MapPin, Calendar, Users, Send, CheckCircle, ArrowRight, MessageSquare, AlertCircle, ShieldCheck } from 'lucide-react';
...
export const EnquiryForm: React.FC<EnquiryFormProps> = ({ target, onSuccess, onClose, isInline = false }) => {
  // ── Form State ──
  const [formData, setFormData] = useState<EnquiryFormData>({
    ...
    address: '',
    travelDate: '',
    travelFlexibility: 'FIXED_DATES',
    preferredTravelDateFrom: '',
  ...
  useEffect(() => {
    ...
    const loadAdminPhone = async () => {
      try {
        // 1. Query authoritative Firestore source first
        ...
        console.log('Firestore settings unreachable, using local cache fallback');
      }
  ...
  useEffect(() => {
    ...
    if (target?.price) {
      setFormData(prev => ({ ...prev, budget: String(target.price), budgetCurrency: target.currency || 'INR' }));
    }
  }, [target]);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.travelFlexibility === 'FIXED_DATES') {
      if (!formData.travelDate) {
        newErrors.travelDate = 'Please select your intended date of travel.';
      }
    ...
    if (formData.adults < 1) {
      newErrors.adults = 'At least 1 adult traveller is required.';
    }
  ...
  const buildWhatsAppUrl = (refIdOverride?: string): string => {
    ...
    const stylesText = (formData.travelStyle && formData.travelStyle.length > 0) ? formData.travelStyle.join(', ') : '';
    const occasionText = formData.specialOccasion === 'Other' ? customOccasion : formData.specialOccasion;
    ...
    if (tripName) {
      lines = [
        ...
        formData.travelDate ? `Travel Date: ${formData.travelDate}` : (formData.preferredTravelDateFrom ? `Travel Range: ${formData.preferredTravelDateFrom} to ${formData.preferredTravelDateTo || 'Flexible'}` : 'Travel Date: Flexible'),
        ...
        formData.budget ? `Budget: ${formData.budgetCurrency || 'INR'} ${formData.budget}` : '',
      ...
      lines = [
        ...
        ``,
        `Reference: ${refId}`,
        formData.travelDate ? `Travel Date: ${formData.travelDate}` : 'Travel Date: Flexible',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const resolvedOccasion = formData.specialOccasion === 'Other' ? customOccasion.trim() : formData.specialOccasion;
    const resolvedDietary = formData.dietary === 'Other' ? customDietary.trim() : formData.dietary;

    const enquiryPayload: Omit<EnquiryDocument, 'id' | 'createdAt' | 'updatedAt'> = {
      enquiryId: refId,
      itineraryId: target?.itineraryId || target?.pkg?.id || '',
      itineraryTitle: target?.itineraryTitle || target?.pkg?.title || 'General Expedition Enquiry',
      ...
      preferences: {
        ...
        travelStyle: formData.travelStyle && formData.travelStyle.length > 0 ? formData.travelStyle : undefined,
      ...
      status: 'NEW',
      source: target?.source || 'ITINERARY',
  ...
  if (submitSuccess) {
    ...
    return (
      <div className="p-8 md:p-12 text-center space-y-6 bg-white border-4 border-[#121212] shadow-[12px_12px_0px_0px_#F4BF4B]">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs font-bold uppercase tracking-wider text-gray-600 max-w-md mx-auto">
            We've received your request for {target?.itineraryTitle || target?.destination || 'your journey'}. Our travel team will review your plans and get in touch with you shortly.
          </p>
        </div>
  ...
  const travelStylesList = ['Adventure', 'Wildlife', 'Culture', 'Food & Wine', 'Wellness', 'Romantic', 'Family', 'Photography', 'Luxury', 'Slow Travel'];
  const interestsList = ['Wildlife', 'Photography', 'Local Food', 'History', 'Nature', 'Art & Culture', 'Wellness'];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 text-left" noValidate>
      ...
      {(target?.pkg || target?.itineraryTitle) && (
        <div className="p-5 bg-[#121212] text-white border-2 border-[#121212] shadow-[6px_6px_0px_0px_#F4BF4B] space-y-2">
          <span className="text-[8px] font-black uppercase tracking-[0.3em] text-[#F4BF4B] block">Selected Expedition</span>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h4 className="font-brand font-black text-xl uppercase tracking-tight text-white leading-tight">
                {target?.itineraryTitle || target?.pkg?.title}
              </h4>
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/60">
                {target?.destination || target?.pkg?.destinations?.[0] || 'Expedition'} {target?.duration ? `• ${target.duration}` : ''}
              </p>
            </div>
            {target?.price && target.price > 0 && (
              <div className="text-left sm:text-right shrink-0">
                <span className="text-[8px] font-black uppercase tracking-widest text-white/40 block">From</span>
      ...
      {/* ── SECTION 1: YOUR JOURNEY & DATES ── */}
      <div className="space-y-4">
        <h4 className="font-brand font-black text-lg uppercase tracking-tight text-[#121212] border-b-2 border-[#121212]/10 pb-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button
                key={item.id}
                type="button"
                onClick={() => setFormData({ ...formData, travelFlexibility: item.id as any })}
                className={`py-3 px-2 text-[10px] font-black uppercase tracking-wider border-2 transition-all cursor-pointer ${formData.travelFlexibility === item.id ? 'bg-[#121212] text-[#F4BF4B] border-[#121212]' : 'bg-white text-[#121212] border-[#121212]/30 hover:border-[#121212]'}`}
              >
                {item.label}
              </button>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full p-3.5 bg-white border-2 font-bold text-xs outline-none transition-colors cursor-pointer ${errors.travelDate ? 'border-rose-600 focus:bg-rose-50' : 'border-[#121212] focus:bg-[#F4BF4B]/10'}`}
              />
              {errors.travelDate && <span className="text-[9px] font-bold text-rose-600 mt-1 block">{errors.travelDate}</span>}
            </div>
            <div>
              <label className="text-[9px] font-black uppercase tracking-[0.2em] text-[#121212]/60 mb-1 block">
                Duration (Days)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                min={new Date().toISOString().split('T')[0]}
                onChange={e => setFormData({ ...formData, preferredTravelDateFrom: e.target.value })}
                className={`w-full p-3.5 bg-white border-2 font-bold text-xs outline-none transition-colors cursor-pointer ${errors.travelDate ? 'border-rose-600 focus:bg-rose-50' : 'border-[#121212] focus:bg-[#F4BF4B]/10'}`}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={e => setFormData({ ...formData, preferredTravelDateTo: e.target.value })}
        ...
        {formData.travelFlexibility === 'VERY_FLEXIBLE' && (
          <p className="p-3 bg-[#FCFBF7] border-2 border-[#121212]/20 text-[10px] font-bold uppercase tracking-wider text-gray-600">
            ℹ That's perfect! Our travel team will suggest optimal departure windows and seasonal dates for your trip.
          </p>
        )}
      </div>
      ...
      <div className="space-y-4 pt-2">
        <div className="flex items-center justify-between border-b-2 border-[#121212]/10 pb-2">
          <h4 className="font-brand font-black text-lg uppercase tracking-tight text-[#121212]">
            2. Who's Travelling
          </h4>
          <span className="text-[10px] font-black uppercase tracking-wider bg-[#121212] text-[#F4BF4B] px-3 py-1">
            {formData.adults} Adults {formData.children > 0 ? `• ${formData.children} Children ` : ''}• {formData.adults + formData.children} Travellers
          </span>
        </div>
        ...
        {formData.children > 0 && (
          <div className="p-4 bg-[#F4BF4B]/10 border-2 border-[#121212] space-y-3">
            ...
            {errors.children && <span className="text-[9px] font-bold text-rose-600 mt-1 block">{errors.children}</span>}
          </div>
        )}
      </div>

      {/* ── SECTION 3: HOW YOU LIKE TO TRAVEL ── */}
      <div className="space-y-4 pt-2">
        <h4 className="font-brand font-black text-lg uppercase tracking-tight text-[#121212] border-b-2 border-[#121212]/10 pb-2">
          3. How You Like to Travel
        </h4>

        {/* Travel Style Tag Selector */}
        <div>
          <label className="text-[9px] font-black uppercase tracking-[0.2em] text-[#121212]/60 mb-2 flex items-center justify-between">
            ...
            <span className="text-[#9E1B1D]">{(formData.travelStyle || []).length}/3 selected</span>
          </label>
            ...
            {travelStylesList.map(style => {
              ...
              return (
                <button
                  ...
                  className={`py-2 px-3.5 text-[10px] font-black uppercase tracking-wider border-2 transition-all cursor-pointer ${isSelected ? 'bg-[#121212] text-[#F4BF4B] border-[#121212]' : 'bg-white text-[#121212]/80 border-[#121212]/30 hover:border-[#121212]'}`}
                >
                  {style}
                </button>
              );
      ...
      {/* ── SECTION 4: PREFERENCES & SPECIAL OCCASION ── */}
      <div className="space-y-4 pt-2">
        <h4 className="font-brand font-black text-lg uppercase tracking-tight text-[#121212] border-b-2 border-[#121212]/10 pb-2">
          4. Preferences & Special Occasion
        </h4>
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Mobility assistance, minimal stair walking..."
            className="w-full p-3.5 bg-white border-2 border-[#121212] font-bold text-xs outline-none focus:bg-[#F4BF4B]/10 transition-colors"
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full p-3.5 bg-white border-2 font-bold text-xs outline-none transition-colors ${errors.name ? 'border-rose-600 focus:bg-rose-50' : 'border-[#121212] focus:bg-[#F4BF4B]/10'}`}
            />
            {errors.name && <span className="text-[9px] font-bold text-rose-600 mt-1 block">{errors.name}</span>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full p-3.5 bg-white border-2 font-bold text-xs outline-none transition-colors ${errors.email ? 'border-rose-600 focus:bg-rose-50' : 'border-[#121212] focus:bg-[#F4BF4B]/10'}`}
            />
            {errors.email && <span className="text-[9px] font-bold text-rose-600 mt-1 block">{errors.email}</span>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full p-3.5 bg-white border-2 font-bold text-xs outline-none transition-colors ${errors.phone ? 'border-rose-600 focus:bg-rose-50' : 'border-[#121212] focus:bg-[#F4BF4B]/10'}`}
            />
            {errors.phone && <span className="text-[9px] font-bold text-rose-600 mt-1 block">{errors.phone}</span>}
          </div>
      ...
      <div className="space-y-4 pt-2">
        ...
        <div>
          ...
          <span className="text-[9px] font-bold text-gray-500 mt-1 block">
            ℹ This helps our travel team select the right stays and private experiences for you.
          </span>
        </div>
      ...
      <div className="pt-4">
        <button
          type="submit"
          disabled={submitting}
          className={`w-full py-4 px-8 bg-[#121212] text-[#F4BF4B] font-black text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-3 transition-all border-2 border-[#121212] shadow-[6px_6px_0px_0px_#F4BF4B] active:translate-x-0.5 active:translate-y-0.5 ${submitting ? 'opacity-60 cursor-not-allowed' : 'hover:bg-[#9E1B1D] hover:text-white cursor-pointer'}`}
        >
          {submitting ? (
            <>SUBMITTING ENQUIRY...</>
          ) : (
